...
using Hk.Core.Util.Offices.Model;
...
namespace Hk.Core.Util.Offices
{
    /// <summary>
    ...
    public class ExcelHelper
    {
        ...
        {
            HttpContext curContext = WebHelper.HttpContext;
            // 设置编码和附件格式
            curContext.Response.ContentType = "application/ms-excel";
            if(curContext.Response.Headers.Keys.Contains("Content-Disposition"))
                curContext.Response.Headers["Content-Disposition"] =
                    "attachment;filename=" + HttpUtility.UrlEncode(excelConfig.FileName, Encoding.UTF8);
            else
            {
                curContext.Response.Headers.Add("Content-Disposition", "attachment;filename=" + HttpUtility.UrlEncode(excelConfig.FileName, Encoding.UTF8));
            }
            //调用导出具体方法Export()
            var buffer = ExportMemoryStream(dtSource, excelConfig).GetBuffer();
            //设置文件的大小10M
            curContext.Response.ContentLength = 10485760;
            curContext.Response.Body.Write(buffer, 0, buffer.Length);
            curContext.Response.Body.Flush();
            curContext.Response.Body.Close();
        }
        /// <summary>
        /// Excel导出下载
        /// </summary>
        ...
        public static MemoryStream ExportListByTempale(List<TemplateMode> list, string templdateName)
        {
            try
            {
                ...
                string templatePath = WebHelper.WebRootPath + "/Resource/ExcelTemplate/";
